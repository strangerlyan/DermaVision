...
def load_model_and_data():
    """Load model menggunakan session_state agar aman dari NaN"""
    
    # 1. Load model ke session_state jika belum ada
    if "cnn_model" not in st.session_state:
        if os.path.exists(MODEL_PATH):
            try:
                # Menggunakan keras.models sesuai dengan 'from tensorflow import keras' di baris atasmu
    ...
    if "class_names" not in st.session_state:
        if os.path.exists(CLASS_PATH):
            ...
            st.error(f"❌ File class_names tidak ditemukan")
            return None, None, None
    ...
    if "meta_cols" not in st.session_state:
        if os.path.exists(META_PATH):
            ...
            st.error(f"❌ File meta_cols tidak ditemukan")
            return None, None, None
